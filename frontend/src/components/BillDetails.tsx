              ...
              <li key={bill.id} className={`cursor-pointer hover:bg-slate-50 ${selectedBill?.id === bill.id ? 'bg-slate-100' : ''}`} onClick={() => onSelectBill(bill)}>
                <div className="p-2 flex justify-between items-center">
                  ...
                  <span className={`font-bold text-lg ${selectedBill?.id === bill.id ? 'text-blue-600' : 'text-slate-800'}`}>
                    {formatCurrency(bill.total_conta_rs)}
                  </span>
                </div>
                    ...
                    <div>
                      <div className="grid grid-cols-[1fr,4fr] items-center mb-1">
                        ...
                        <div className="grid grid-cols-5 gap-2 text-xs font-bold text-slate-400 text-right">
                          <span>Consumo</span><span>Faixa</span><span>Tarifa / m³</span><span>Deduzir</span><span>Sub-Total</span>
                        </div>
                      </div>
                      <BillRow label="Esgoto">
                        ...
                        <span className="text-right">{formatCurrency(selectedBill.tarifa_esgoto)}</span>
                        <span className="text-right">{formatCurrency(selectedBill.deduzir_esgoto)}</span>
                        <span className="text-right font-bold text-slate-800">{formatCurrency(selectedBill.total_esgoto_rs)}</span>
                      </BillRow>
                      <BillRow label="Água Produzida">
                        ...
                        <span className="text-right">{formatCurrency(selectedBill.tarifa_agua)}</span>
                        <span className="text-right">{formatCurrency(selectedBill.deduzir_agua)}</span>
                        <span className="text-right font-bold text-slate-800">{formatCurrency(selectedBill.cobrado_agua_prod_rs)}</span>
                      </BillRow>
                      <BillRow label="Água Comprada">
                        <span className="text-right">{selectedBill.consumo_comprado_m3} m³</span>
                        <span className="text-right"></span>
                        <span className="text-right">{formatCurrency(selectedBill.preco_m3_comprado_rs)}</span>
                        <span className="text-right"></span>
                        <span className="text-right font-bold text-slate-800">{formatCurrency(selectedBill.cobrado_agua_comp_rs)}</span>
                      </BillRow>
                      <BillRow label="Area Comum">
                        <span className="col-span-5 text-right font-bold text-slate-800">{formatCurrency(selectedBill.cobrado_area_comum_rs)}</span>
                      </BillRow>
                      <BillRow label="Outros Gastos">
                        <span className="col-span-5 text-right font-bold text-slate-800">{formatCurrency(selectedBill.cobrado_outros_gastos_rs)}</span>
                      </BillRow>
                    </div>
                  </div>
                )}
