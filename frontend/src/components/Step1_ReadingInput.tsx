...
const Step1_ReadingInput: React.FC<Step1Props> = ({ state, dispatch }) => {
  ...
  const getInputStyles = (consumo: number | null, leitura_atual: number | null): string => {
    const base = "w-full p-1.5 border rounded-md focus:ring-2 text-sm";
    ...
    <div className="col-span-3 flex flex-col overflow-y-auto">
      <div className="mb-2 p-2 border rounded-lg bg-slate-50">
        <div className="grid grid-cols-1 md:grid-cols-7 gap-2 items-end">
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Data Referência</label><input type="text" value={state.productionData.data_ref ?? ''} className="w-full p-2 border rounded-md text-sm bg-slate-200" disabled /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Produção m³</label><input type="number" value={state.productionData.producao_m3 ?? ''} onChange={(e) => handleProductionDataChange('producao_m3', e.target.value)} className="w-full p-2 border rounded-md text-sm" /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Compra Água R$</label><input type="number" value={state.productionData.compra_rs ?? ''} onChange={(e) => handleProductionDataChange('compra_rs', e.target.value)} className="w-full p-2 border rounded-md text-sm" /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Outros Gastos R$</label><input type="number" value={state.productionData.outros_rs ?? ''} onChange={(e) => handleProductionDataChange('outros_rs', e.target.value)} className="w-full p-2 border rounded-md text-sm" /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Total m³</label><input type="number" value={state.productionData.total_consumo_m3 ?? ''} className="w-full p-2 border rounded-md bg-slate-200 text-sm" disabled /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Média m³</label><input type="number" value={state.productionData.media_m3 ?? ''} className="w-full p-2 border rounded-md bg-slate-200 text-sm" disabled /></div>
          <div><label className="block text-xs font-medium text-slate-700 mb-1">Mediana m³</label><input type="number" value={state.productionData.mediana_m3 ?? ''} className="w-full p-2 border rounded-md bg-slate-200 text-sm" disabled /></div>
        </div>
      </div>
