...
// Define os tipos para as visões possíveis, que devem ser os mesmos do App.tsx
...
const Header: React.FC<HeaderProps> = ({ onLogout, userName, userProfile, onOpenReadings, activeView, onNavigate }) => {
  // Função para gerar as classes CSS com base na visão ativa
  const getLinkClass = (view: ActiveView) => {
    const baseClass = "font-medium pb-1";
    if (activeView === view) {
      return `${baseClass} text-blue-600 font-bold border-b-2 border-blue-600`;
    }
  ...
  return (
    <header className="bg-white shadow-md p-4 flex justify-between items-center">
      ...
      <nav className="flex items-center space-x-6">
        ...
        <button onClick={() => onNavigate('painel')} className={getLinkClass('painel')}>
          Painel
        </button>
      </nav>
